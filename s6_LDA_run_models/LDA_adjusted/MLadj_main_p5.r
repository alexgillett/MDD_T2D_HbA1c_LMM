...
### Run, using ML, the (main/ only) adjusted model
fit_adj_mainML5 <- with(mi_list5,
...
correlation = corCAR1(form=~time_base|eid_f), method="ML"),
include.data=TRUE)

rm(hba1c.imp_srethn, mi_list5)

### save output
save.image(file=paste(analysis_dir, "model_output/adjusted_mainML5.RData", sep=""))
