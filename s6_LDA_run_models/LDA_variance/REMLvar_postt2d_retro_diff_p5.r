...
### Run model:
fit_postt2dretro_diff_REML5 <- with(mi_list5,
...
correlation = corCAR1(form=~time_base|eid_f), weights=varIdent(form=~1|t2d_dep), method="REML"),
include.data=TRUE)

rm(hba1c.imp_srethn, mi_list5)

### save output
save.image(file=paste(analysis_dir, "model_output/var_postt2dretro_diffREML5.RData", sep=""))
