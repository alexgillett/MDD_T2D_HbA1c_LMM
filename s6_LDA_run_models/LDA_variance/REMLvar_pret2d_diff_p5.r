...
### Run model:
fit_pret2d_diff_REML5 <- with(mi_list5,
...
correlation = corCAR1(form=~time_base|eid_f), weights=varIdent(form=~1|dep_base), method="REML"),
include.data=TRUE)

rm(hba1c.imp_srethn, mi_list5)

### save output
save.image(file=paste(analysis_dir, "model_output/var_pret2d_diffREML5.RData", sep=""))
