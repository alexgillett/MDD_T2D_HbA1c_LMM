...
### Extract the 10 imputed datasets required (second set of 10 here)
for(i in 1:10){
  # assign function within loop
  assign(paste0("df_", i), hba1c.imp_srethn$imputations[[i+10]])
}
...
mi_list2 <- long2mitml.list(df_mi, split = "mi_id", exclude = 0)
rm(df_mi)

### Run model:
fit_postt2dTV_diff_REML2 <- with(mi_list2,
...
rm(hba1c.imp_srethn, mi_list2)

### save output
save.image(file=paste(analysis_dir, "model_output/var_postt2dTV_diffREML2.RData", sep=""))
