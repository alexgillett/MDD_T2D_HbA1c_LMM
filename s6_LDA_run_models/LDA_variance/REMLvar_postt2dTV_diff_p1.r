...
### Extract the 10 imputed datasets required (first 10 here)
for(i in 1:10){
...
mi_list1 <- long2mitml.list(df_mi, split = "mi_id", exclude = 0)
rm(df_mi)

### Run model:
fit_postt2dTV_diff_REML1 <- with(mi_list1,
...
rm(hba1c.imp_srethn, mi_list1)

### save output
save.image(file=paste(analysis_dir, "model_output/var_postt2dTV_diffREML1.RData", sep=""))
