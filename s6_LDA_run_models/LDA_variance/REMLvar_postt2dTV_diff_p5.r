...
### Extract the 10 imputed datasets required (fifth/last set of 10 here)
for(i in 1:10){
  # assign function within loop
  assign(paste0("df_", i), hba1c.imp_srethn$imputations[[i+40]])
}
...
mi_list5 <- long2mitml.list(df_mi, split = "mi_id", exclude = 0)
rm(df_mi)

### Run model:
fit_postt2dTV_diff_REML5 <- with(mi_list5,
...
correlation = corCAR1(form=~time_base|eid_f), weights=varIdent(form=~1|dep_change_base), method="REML"),
include.data=TRUE)

rm(hba1c.imp_srethn, mi_list5)

### save output
save.image(file=paste(analysis_dir, "model_output/var_postt2dTV_diffREML5.RData", sep=""))
